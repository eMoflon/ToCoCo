...
static void print_powercontrol() {
	...
	for(nexthop = list_head(components_network_nexthops_all()); nexthop != NULL; nexthop = list_item_next(nexthop)) {
		printf("%s=%d", networkaddr2string_buffered(nexthop->address), components_powercontrol_destinationtxpower(nexthop->address));
		if(nexthop->next != NULL)
			printf(", ");
	}
	printf("\n");
...
void evaluation_print() {
	...
	printf("[evaluation] basestation-nexthop=%s\n", networkaddr2string_buffered(components_network_nexthops_basestation()));
#endif

...
PROCESS_THREAD(component_application, ev, data) {
	...
	component_network_packet_subscribe(messagetype_broadcast, recv_broadcast);
	component_network_packet_subscribe(messagetype_hopwalk, recv_hopwalk);
	component_network_packet_subscribe(messagetype_routing, recv_routing);
	...
	while(1) {
		...
		if(etimer_expired(&etimer_broadcast)) {
			etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_APPLICATION_RANDOMWALK_INTERVALBROADCAST_MIN, CLOCK_SECOND * COMPONENT_APPLICATION_RANDOMWALK_INTERVALBROADCAST_MAX));
			component_network_packet_send(COMPONENT_NETWORK_TRANSMISSION_LINKLOCAL_BROADCAST, messagetype_broadcast, NULL, component_network_packet_sendbuffer(), -1, -1);
		}
...
static void recv_hopwalk(const networkaddr_t *source, buffer_t *data, int8_t rssi) {
	...
	if(--hopwalk_count > 0) {
		...
		if(networkaddr_equal(&hopwalk_source, networkaddr_node_addr())) {
			...
			component_network_packet_send(COMPONENT_NETWORK_TRANSMISSION_ROUTING_UNICAST, messagetype_routing, &hopwalk_source, component_network_packet_sendbuffer(), -1, -1);
		}
	}
}
...
void sendToRandomNeighbor(const networkaddr_t *hopwalk_source, uint8_t hopwalk_count) {
	...
	for(i = 0; i < sizeof(neighbors) / sizeof(neighbors[0]); i++) {
		...
		if(numNeighbor++ == randomNeighbor) {
			...
			component_network_packet_send(COMPONENT_NETWORK_TRANSMISSION_LINKLOCAL_UNICAST, messagetype_hopwalk, neighbors[i].address, data, -1, -1);

...
PROCESS_THREAD(component_application, ev, data) {
	...
	while(testsamples_remaining-- > 0) {
		...
		for(powerlevel_i = 0; powerlevel_i < POWERLEVELS; powerlevel_i++) {
			...
			// broadcasting
			buffer_t *sendbuffer = components_network_packet_sendbuffer();
			buffer_append_int8t(sendbuffer, powerlevels[powerlevel_i]);
			components_network_packet_send(COMPONENTS_NETWORK_TRANSMISSION_LINKLOCAL_BROADCAST, NULL, powerlevels[powerlevel_i], messagetype, sendbuffer);
		}
